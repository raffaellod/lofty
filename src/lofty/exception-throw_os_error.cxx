...
#if LOFTY_HOST_API_POSIX
   LOFTY_ASSERT(err != 0, LOFTY_SL("cannot throw an exception for a success"));
   switch (err) {
...
// These two values may or may not be different.
      ...
      case ENOMEM: // Out of memory (POSIX.1-2001)
         LOFTY_THROW(memory::bad_alloc, (0, err));

      case EFAULT: // Bad address (POSIX.1-2001)
         LOFTY_THROW(memory::bad_pointer, (err));
...
#endif
         LOFTY_THROW(network_error, (err));

      case ENAMETOOLONG: // File name too long (POSIX.1-2001)
      case ENOTDIR: // Not a directory (POSIX.1-2001)
         LOFTY_THROW(os::invalid_path, (os::path(LOFTY_SL("<not available>")), err));

      case ENODEV: // No such device (POSIX.1-2001)
      case ENOENT: // No such file or directory (POSIX.1-2001)
         LOFTY_THROW(os::path_not_found, (os::path(LOFTY_SL("<not available>")), err));

      case EACCES: // Permission denied (POSIX.1-2001)
      case EPERM: // Operation not permitted (POSIX.1-2001)
         LOFTY_THROW(security_error, (err));
   }
#elif LOFTY_HOST_API_WIN32
   LOFTY_ASSERT(err != ERROR_SUCCESS, LOFTY_SL("cannot throw an exception for a success"));
   switch (err) {
      /* TODO: Win32 defines these “positive failures”: what to do? They’re not generic_error’s, so we
      shouldn’t throw due to them. */
      case ERROR_SUCCESS_REBOOT_REQUIRED: /* The requested operation is successful. Changes will not be
         effective until the system is rebooted. */
      case ERROR_SUCCESS_RESTART_REQUIRED: /* The requested operation is successful. Changes will not be
         effective until the service is restarted. */
         break;

      case ERROR_BAD_DESCRIPTOR_FORMAT: /* A security descriptor is not in the right format (absolute or
         self-relative). */
      ...
      case ERROR_INVALID_AT_INTERRUPT_TIME: // Cannot request exclusive semaphores at interrupt time.
      case ERROR_INVALID_BLOCK: // The storage control block address is invalid.
      case ERROR_INVALID_BLOCK_LENGTH: /* When accessing a new tape of a multivolume partition, the current
         block size is incorrect. */
      case ERROR_INVALID_CATEGORY: // The IOCTL call made by the application program is not correct.
      case ERROR_INVALID_COMBOBOX_MESSAGE: /* Invalid message for a combo box because it does not have an edit
         control. */
      ...
      case ERROR_INVALID_EVENT_COUNT: /* The number of specified semaphore events for DosMuxSemWait is not
         correct. */
      ...
      case ERROR_INVALID_GROUP_ATTRIBUTES: /* The specified attributes are invalid, or incompatible with the
      ...
      case ERROR_INVALID_ID_AUTHORITY: // The value provided was an invalid value for an identifier authority.
      ...
      case ERROR_INVALID_PRINT_MONITOR: // The specified print monitor does not have the required functions.
      ...
      case ERROR_INVALID_SEGMENT_NUMBER: // The system detected a segment number that was not correct.
      ...
      case ERROR_INVALID_VERIFY_SWITCH: // The verify-on-write switch parameter value is not correct.
      case ERROR_INVALID_WINDOW_HANDLE: // Invalid window handle.
      case ERROR_INVALID_WINDOW_STYLE: // The window style or class attribute is invalid for this operation.
      ...
      case ERROR_ALREADY_INITIALIZED: /* An attempt was made to perform an initialization operation when
      ...
      case ERROR_BOOT_ALREADY_ACCEPTED: /* The current boot has already been accepted for use as the last-
      ...
      case ERROR_CAN_NOT_DEL_LOCAL_WINS: /* The local Windows Internet Naming Service (WINS) cannot be
         deleted. */
      ...
      case ERROR_CANT_ACCESS_DOMAIN_INFO: /* Indicates that a Windows NT Server could not be contacted or that
         objects within the domain are protected such that necessary information could not be retrieved. */
      case ERROR_CHILD_MUST_BE_VOLATILE: // Cannot create a stable subkey under a volatile parent key.
      ...
      case ERROR_DEPENDENT_SERVICES_RUNNING: /* A stop control has been sent to a service that other running
         services are dependent on. */
      case ERROR_DESTROY_OBJECT_OF_OTHER_THREAD: // Cannot destroy object created by another thread.
      case ERROR_DIFFERENT_SERVICE_ACCOUNT: /* The account specified for this service is different from the
      ...
      case ERROR_DLL_NOT_FOUND: // One of the library files needed to run this application cannot be found.
      case ERROR_DUP_DOMAINNAME: /* The workgroup or domain name is already in use by another computer on the
         network. */
      case ERROR_DUP_NAME: // A duplicate name exists on the network.
      case ERROR_DUPLICATE_SERVICE_NAME: /* The name is already in use as either a service name or a service
         display name. */
      case ERROR_DYNLINK_FROM_INVALID_RING: // The operating system cannot run this application program.
      case ERROR_ENVVAR_NOT_FOUND: // The system could not find the environment option that was entered.
      case ERROR_EVENTLOG_CANT_START: /* No event tracking file could be opened, so the event tracking service
         did not start. */
      case ERROR_EVENTLOG_FILE_CHANGED: // The event tracking file has changed between read operations.
      case ERROR_EVENTLOG_FILE_CORRUPT: // The event tracking file is corrupted.
      case ERROR_EXCEPTION_IN_SERVICE: /* An exception occurred in the service when handling the control
         request. */
      case ERROR_EXCL_SEM_ALREADY_OWNED: // The exclusive semaphore is owned by another process.
      case ERROR_EXE_MACHINE_TYPE_MISMATCH: /* The image file %1 is valid, but is for a machine type other
         than the current machine. */
      case ERROR_EXE_MARKED_INVALID: // The operating system cannot run %1.
      case ERROR_FULLSCREEN_MODE: // The requested operation cannot be performed in full-screen mode.
      ...
      case ERROR_HWNDS_HAVE_DIFF_PARENT: /* All handles to windows in a multiple-window position structure
         must have the same parent. */
      case ERROR_INC_BACKUP: // The backup failed. Was a full backup done before?
      case ERROR_INCORRECT_ADDRESS: // The network address could not be used for the operation requested.
      case ERROR_INFLOOP_IN_RELOC_CHAIN: // The operating system cannot run %1.
      case ERROR_INTERNAL_DB_CORRUPTION: /* Unable to complete the requested operation because of either a
      ...
      case ERROR_INTERNAL_DB_ERROR: /* The local security authority database contains an internal
         inconsistency. */
      case ERROR_INTERNAL_ERROR: // The security account database contains an internal inconsistency.
      ...
      case ERROR_LICENSE_QUOTA_EXCEEDED: /* The service being accessed is licensed for a particular number of
         connections. No more connections can be made to the service at this time because there are already as
         many connections as the service can accept. */
      case ERROR_LISTBOX_ID_NOT_FOUND: // The list box identifier was not found.
      case ERROR_LOCK_VIOLATION: /* The process cannot access the file because another process has locked a
         portion of the file. */
      ...
      case ERROR_META_EXPANSION_TOO_LONG: /* The global file name characters, "*" or "?," are entered
      ...
      case ERROR_MR_MID_NOT_FOUND: /* The system cannot find message text for message number 0x%1 in the
         message file for %2. */
      ...
      case ERROR_NO_SHUTDOWN_IN_PROGRESS: /* Unable to abort the system shutdown because no shutdown was in
         progress. */
      case ERROR_NO_SIGNAL_SENT: // No process in the command subtree has a signal handler.
      case ERROR_NO_SPOOL_SPACE: /* Space to store the file waiting to be printed is not available on the
         server. */
      ...
      case ERROR_NOLOGON_INTERDOMAIN_TRUST_ACCOUNT: /* The account used is an interdomain trust account. Use
         your global user account or local user account to access this server. */
      case ERROR_NOLOGON_SERVER_TRUST_ACCOUNT: /* The account used is a server trust account. Use your global
         user account or local user account to access this server. */
      case ERROR_NOLOGON_WORKSTATION_TRUST_ACCOUNT: /* The account used is a computer account. Use your global
         user account or local user account to access this server. */
      case ERROR_NON_MDICHILD_WINDOW: /* Cannot process a message from a window that is not a multiple-
      ...
      case ERROR_PORT_UNREACHABLE: /* No service is operating at the destination network endpoint on the
         remote system. */
      ...
      case ERROR_REMOTE_SESSION_LIMIT_EXCEEDED: /* An attempt was made to establish a session to a network
         server, but there are already too many sessions established to that server. */
      case ERROR_REQUIRES_INTERACTIVE_WINDOWSTATION: // This operation requires an interactive window station.
      case ERROR_RESOURCE_DATA_NOT_FOUND: // The specified image file did not contain a resource section.
      case ERROR_RESOURCE_LANG_NOT_FOUND: /* The specified resource language identifier cannot be found in the
         image file. */
      case ERROR_RESOURCE_NAME_NOT_FOUND: // The specified resource name cannot be found in the image file.
      case ERROR_RESOURCE_TYPE_NOT_FOUND: // The specified resource type cannot be found in the image file.
      ...
      case ERROR_RING2SEG_MUST_BE_MOVABLE: // The code segment cannot be greater than or equal to 64 KB.
      ...
      case ERROR_SERVICE_CANNOT_ACCEPT_CTRL: // The service cannot accept control messages at this time.
      case ERROR_SERVICE_DATABASE_LOCKED: // The service database is locked.
      case ERROR_SERVICE_DEPENDENCY_DELETED: /* The dependency service does not exist or has been marked for
         deletion. */
      ...
      case ERROR_SERVICE_DOES_NOT_EXIST: // The specified service does not exist as an installed service.
      ...
      case ERROR_SERVICE_NEVER_STARTED: /* No attempts to start the service have been made since the last
         boot. */
      ...
      case ERROR_SERVICE_START_HANG: /* After starting, the service stopped responding in a start-pending
         state. */
      case ERROR_SESSION_CREDENTIAL_CONFLICT: /* The credentials supplied conflict with an existing set of
         credentials. */
      case ERROR_SET_POWER_STATE_FAILED: /* The basic input/output system (BIOS) failed an attempt to change
         the system power state. */
      case ERROR_SET_POWER_STATE_VETOED: /* An attempt to change the system power state was vetoed by another
      ...
      case ERROR_SINGLE_INSTANCE_APP: // Cannot start more than one instance of the specified program.
      ...
      case ERROR_TRANSFORM_NOT_SUPPORTED: // The requested transformation operation is not supported.
      ...
      case ERROR_ADDRESS_NOT_ASSOCIATED: // An address has not yet been associated with the network endpoint.
      ...
      case ERROR_ATOMIC_LOCKS_NOT_SUPPORTED: /* The file system does not support atomic changes to the lock
         type. */
      ...
      case ERROR_DEVICE_ALREADY_REMEMBERED: /* An attempt was made to remember a device that had previously
         been remembered. */
      case ERROR_DEVICE_IN_USE: // The device is in use by an active process and cannot be disconnected.
      case ERROR_DEVICE_NOT_PARTITIONED: // Tape partition information could not be found when loading a tape.
      ...
      case ERROR_DIRECT_ACCESS_HANDLE: /* Attempt to use a file handle to an open disk partition for an
      ...
      case ERROR_DISK_OPERATION_FAILED: /* While accessing the hard disk, a disk operation failed even after
         retries. */
      case ERROR_DISK_RECALIBRATE_FAILED: /* While accessing the hard disk, a recalibrate operation failed,
         even after retries. */
      case ERROR_DISK_RESET_FAILED: /* While accessing the hard disk, a disk controller reset was needed, but
         even that failed. */
      case ERROR_DRIVE_LOCKED: // The disk is in use or locked by another process.
      case ERROR_EA_FILE_CORRUPT: // The extended attribute file on the mounted file system is corrupt.
      ...
      case ERROR_FILE_INVALID: /* The volume for a file has been externally altered so that the opened file is
         no longer valid. */
      ...
      case ERROR_FLOPPY_BAD_REGISTERS: /* The floppy disk controller returned inconsistent results in its
         registers. */
      case ERROR_FLOPPY_ID_MARK_NOT_FOUND: // No identifier address mark was found on the floppy disk.
      case ERROR_FLOPPY_UNKNOWN_ERROR: /* The floppy disk controller reported an error that is not recognized
         by the floppy disk driver. */
      case ERROR_FLOPPY_WRONG_CYLINDER: /* Mismatch between the floppy disk sector identifier field and the
         floppy disk controller track address. */
      case ERROR_FULL_BACKUP: // The backup failed. Check the directory to which you are backing the database.
      case ERROR_HANDLE_DISK_FULL: // The disk is full.
      case ERROR_HANDLE_EOF: // Reached the end of the file.
      case ERROR_INVALID_USER_BUFFER: // The supplied user buffer is not valid for the requested operation.
      ...
      case ERROR_IRQ_BUSY: /* Unable to open a device that was sharing an interrupt request (IRQ) with other
      ...
      case ERROR_IS_JOINED: /* An attempt was made to use a JOIN or SUBST command on a drive that has already
         been joined. */
      case ERROR_IS_SUBST_PATH: // The path specified is being used in a substitute.
      case ERROR_IS_SUBST_TARGET: /* An attempt was made to join or substitute a drive for which a directory
         on the drive is the target of a previous substitute. */
      case ERROR_IS_SUBSTED: /* An attempt was made to use a JOIN or SUBST command on a drive that has already
         been substituted. */
      case ERROR_JOIN_TO_JOIN: // The system tried to join a drive to a directory on a joined drive.
      case ERROR_JOIN_TO_SUBST: // The system tried to join a drive to a directory on a substituted drive.
      case ERROR_JOURNAL_HOOK_SET: // The journal hook procedure is already installed.
      case ERROR_KEY_DELETED: /* Illegal operation attempted on a registry key that has been marked for
         deletion. */
      case ERROR_KEY_HAS_CHILDREN: /* Cannot create a symbolic link in a registry key that already has subkeys
         or values. */
      case ERROR_LABEL_TOO_LONG: /* The volume label you entered exceeds the label character limit of the
         target file system. */
      case ERROR_LOCK_FAILED: // Unable to lock a region of a file.
      case ERROR_MAPPED_ALIGNMENT: /* The base address or the file offset specified does not have the proper
         alignment. */
      case ERROR_MORE_WRITES: /* A serial I/O operation was completed by another write to the serial port. The
         IOCTL_SERIAL_XOFF_COUNTER reached zero. */
      case ERROR_NEGATIVE_SEEK: /* An attempt was made to move the file pointer before the beginning of the
         file. */
      ...
      case ERROR_NOT_ENOUGH_SERVER_MEMORY: // Not enough server storage is available to process this command.
      ...
      case ERROR_NOT_REGISTRY_FILE: /* The system has attempted to load or restore a file into the registry,
      ...
      case ERROR_NOT_SUBSTED: /* The system tried to delete the substitution of a drive that is not
         substituted. */
      case ERROR_NOTIFY_ENUM_DIR: /* A notify change request is being completed and the information is not
         being returned in the caller's buffer. The caller now needs to enumerate the files to find the
         changes. */
      case ERROR_OPEN_FAILED: // The system cannot open the device or file specified.
      case ERROR_OPERATION_ABORTED: /* The I/O operation has been aborted because of either a thread exit or
      ...
      case ERROR_PRINT_MONITOR_ALREADY_INSTALLED: // The specified print monitor has already been installed.
      case ERROR_PRINT_MONITOR_IN_USE: // The specified print monitor is currently in use.
      case ERROR_PRINT_PROCESSOR_ALREADY_INSTALLED: /* The specified print processor has already been
         installed. */
      ...
      case ERROR_PRINTER_DRIVER_ALREADY_INSTALLED: // The specified printer driver is already installed.
      case ERROR_PRINTER_DRIVER_IN_USE: // The specified printer driver is currently in use.
      case ERROR_PRINTER_HAS_JOBS_QUEUED: /* The requested operation is not allowed when there are jobs queued
         to the printer. */
      ...
      case ERROR_REGISTRY_CORRUPT: /* The registry is corrupted. The structure of one of the files that
         contains registry data is corrupted, or the system's image of the file in memory is corrupted, or the
         file could not be recovered because the alternate copy or log was absent or corrupted. */
      case ERROR_REGISTRY_IO_FAILED: /* An I/O operation initiated by the registry failed unrecoverably. The
         registry could not read in, or write out, or flush, one of the files that contain the system's image
         of the registry. */
      case ERROR_REGISTRY_RECOVERED: /* One of the files in the registry database had to be recovered by use
         of a log or alternate copy. The recovery was successful. */
      case ERROR_REQUEST_ABORTED: // The request was aborted.
      case ERROR_RXACT_COMMIT_FAILURE: // An internal security database corruption has been encountered.
      case ERROR_RXACT_INVALID_STATE: /* The transaction state of a registry subtree is incompatible with the
         requested operation. */
      case ERROR_SAME_DRIVE: /* The system cannot join or substitute a drive to or for a directory on the same
         drive. */
      ...
      case ERROR_SERIAL_NO_DEVICE: /* No serial device was successfully initialized. The serial driver will
         unload. */
      ...
      case ERROR_SUBST_TO_JOIN: // The system tried to SUBST a drive to a directory on a joined drive.
      case ERROR_SUBST_TO_SUBST: /* The system tried to substitute a drive to a directory on a substituted
         drive. */
      case ERROR_TOO_MANY_LINKS: /* An attempt was made to create more links on a file than the file system
         supports. */
      case ERROR_TOO_MANY_OPEN_FILES: // The system cannot open the file.
      case ERROR_USER_MAPPED_FILE: /* The requested operation cannot be performed on a file with a user-mapped
         section open. */
      ...
      case ERROR_UNRECOGNIZED_VOLUME: /* The volume does not contain a recognized file system. Verify that all
         required file system drivers are loaded and that the volume is not corrupted. */
      ...
      case ERROR_NO_SYSTEM_RESOURCES: /* Insufficient system resources exist to complete the requested
         service. */
      case ERROR_NONPAGED_SYSTEM_RESOURCES: /* Insufficient system resources exist to complete the requested
         service. */
      ...
      case ERROR_PAGED_SYSTEM_RESOURCES: /* Insufficient system resources exist to complete the requested
         service. */
         LOFTY_THROW(memory::bad_alloc, (0, err));
      ...
      case ERROR_ADDRESS_ALREADY_ASSOCIATED: /* The network transport endpoint already has an address
         associated with it. */
      case ERROR_CONNECTION_ACTIVE: // An invalid operation was attempted on an active network connection.
      case ERROR_CONNECTION_COUNT_LIMIT: /* A connection to the server could not be made because the limit on
         the number of concurrent connections for this account has been reached. */
      case ERROR_CONNECTION_UNAVAIL: /* The device is not currently connected but it is a remembered
         connection. */
      case ERROR_DOMAIN_CONTROLLER_NOT_FOUND: // Could not find the domain controller for this domain.
      case ERROR_DOMAIN_EXISTS: // The specified domain already exists.
      case ERROR_DOMAIN_LIMIT_EXCEEDED: /* An attempt was made to exceed the limit on the number of domains
         per server. */
      case ERROR_NETLOGON_NOT_STARTED: /* An attempt was made to logon, but the network logon service was not
         started. */
      ...
      case ERROR_NO_BROWSER_SERVERS_FOUND: /* The list of servers for this workgroup is not currently
         available. */
      case ERROR_NO_LOGON_SERVERS: /* There are currently no logon servers available to service the logon
         request. */
      case ERROR_NO_NETWORK: // The network is not present or not started.
      case ERROR_NOT_LOGGED_ON: /* The operation being requested was not performed because the user has not
      ...
      case ERROR_TOO_MANY_NAMES: // The name limit for the local computer network adapter card was exceeded.
      ...
      case ERROR_ACCOUNT_LOCKED_OUT: /* The referenced account is currently locked out and may not be logged
         on to. */
      ...
      case ERROR_ALLOTTED_SPACE_EXCEEDED: // No more memory is available for security information updates.
      case ERROR_BAD_IMPERSONATION_LEVEL: /* Either a required impersonation level was not provided, or the
         provided impersonation level is invalid. */
      case ERROR_BAD_INHERITANCE_ACL: /* The inherited access control list (ACL) or access control entry (ACE)
         could not be built. */
      case ERROR_BAD_LOGON_SESSION_STATE: /* The logon session is not in a state that is consistent with the
         requested operation. */
      ...
      case ERROR_DOMAIN_TRUST_INCONSISTENT: /* The name or security identifier (SID) of the domain specified
      ...
      case ERROR_GENERIC_NOT_MAPPED: /* Generic access types were contained in an access mask that should
         already be mapped to nongeneric types. */
      case ERROR_ILL_FORMED_PASSWORD: /* Unable to update the password. The value provided for the new
         password contains values that are not allowed in passwords. */
      case ERROR_INVALID_DOMAIN_ROLE: /* This operation is only allowed for the Primary Domain Controller
         (PDC) of the domain. */
      case ERROR_INVALID_DOMAIN_STATE: // The domain was in the wrong state to perform the security operation.
      case ERROR_INVALID_LOGON_HOURS: // Logon failure - account logon time restriction violation.
      case ERROR_INVALID_MEMBER: /* A new member could not be added to a local group because the member has
         the wrong account type. */
      case ERROR_INVALID_OWNER: // This security identifier may not be assigned as the owner of this object.
      case ERROR_INVALID_PASSWORD: // The specified network password is not correct.
      case ERROR_INVALID_PRIMARY_GROUP: /* This security identifier may not be assigned as the primary group
         of an object. */
      case ERROR_INVALID_SERVER_STATE: /* The security account manager (SAM) or local security authority (LSA)
      ...
      case ERROR_INVALID_SUB_AUTHORITY: /* The subauthority part of a security identifier is invalid for this
         particular use. */
      case ERROR_INVALID_WORKSTATION: // Logon failure - user not allowed to log on to this computer.
      case ERROR_LAST_ADMIN: // The last remaining administration account cannot be disabled or deleted.
      case ERROR_LM_CROSS_ENCRYPTION_REQUIRED: /* A cross-encrypted password is necessary to change this user
         password. */
      case ERROR_LOCAL_USER_SESSION_KEY: /* No encryption key is available. A well-known encryption key was
         returned. */
      case ERROR_LOGIN_TIME_RESTRICTION: /* Attempting to log in during an unauthorized time of day for this
         account. */
      case ERROR_LOGIN_WKSTA_RESTRICTION: // The account is not authorized to log in from this station.
      case ERROR_LOGON_FAILURE: // Logon failure - unknown user name or bad password.
      case ERROR_LOGON_NOT_GRANTED: /* Logon failure - the user has not been granted the requested logon type
         at this computer. */
      case ERROR_LOGON_SESSION_COLLISION: // The logon session identifier is already in use.
      case ERROR_LOGON_SESSION_EXISTS: /* Cannot start a new logon session with an identifier that is already
         in use. */
      case ERROR_LOGON_TYPE_NOT_GRANTED: /* Logon failure - the user has not been granted the requested logon
         type at this computer. */
      case ERROR_MEMBER_IN_ALIAS: // The specified account name is already a member of the local group.
      case ERROR_MEMBER_IN_GROUP: /* Either the specified user account is already a member of the specified
         group, or the specified group cannot be deleted because it contains a member. */
      case ERROR_MEMBER_NOT_IN_ALIAS: // The specified account name is not a member of the local group.
      case ERROR_MEMBER_NOT_IN_GROUP: /* The specified user account is not a member of the specified group
         account. */
      case ERROR_MEMBERS_PRIMARY_GROUP: /* The user cannot be removed from a group because the group is
         currently the user's primary group. */
      case ERROR_NO_ASSOCIATION: // No application is associated with the specified file for this operation.
      case ERROR_NO_IMPERSONATION_TOKEN: /* An attempt has been made to operate on an impersonation token by a
      ...
      case ERROR_NO_QUOTAS_FOR_ACCOUNT: // No system quota limits are specifically set for this account.
      case ERROR_NO_SECURITY_ON_OBJECT: /* Unable to perform a security operation on an object that has no
         associated security. */
      ...
      case ERROR_NO_SUCH_LOGON_SESSION: /* A specified logon session does not exist. It may already have been
         terminated. */
      case ERROR_NO_SUCH_MEMBER: /* A new member could not be added to a local group because the member does
         not exist. */
      ...
      case ERROR_NO_TRUST_SAM_ACCOUNT: /* The Security access Model (SAM) database on the Windows NT Server
         does not have a computer account for this workstation trust relationship. */
      case ERROR_NO_USER_SESSION_KEY: // There is no user session key for the specified logon session.
      case ERROR_NOT_ALL_ASSIGNED: // Not all privileges referenced are assigned to the caller.
      case ERROR_NOT_AUTHENTICATED: /* The operation being requested was not performed because the user has
         not been authenticated. */
      case ERROR_NOT_LOGON_PROCESS: /* The requested action is restricted for use by logon processes only. The
         calling process has not registered as a logon process. */
      case ERROR_NT_CROSS_ENCRYPTION_REQUIRED: /* A cross-encrypted password is necessary to change a user
         password. */
      case ERROR_NULL_LM_PASSWORD: /* The password is too complex to be converted to a LAN Manager password.
      ...
      case ERROR_PASSWORD_MUST_CHANGE: // The user must change his password before he logs on the first time.
      case ERROR_PASSWORD_RESTRICTION: /* Unable to update the password because a password update rule has
         been violated. */
      case ERROR_SHARING_VIOLATION: /* The process cannot access the file because it is being used by another
         process. */
      ...
      case ERROR_TRUSTED_DOMAIN_FAILURE: /* The trust relationship between the primary domain and the trusted
         domain failed. */
      case ERROR_TRUSTED_RELATIONSHIP_FAILURE: /* The trust relationship between this workstation and the
      ...
      case ERROR_WRONG_PASSWORD: /* Unable to update the password. The value provided as the current password
         is incorrect. */
         LOFTY_THROW(security_error, (err));

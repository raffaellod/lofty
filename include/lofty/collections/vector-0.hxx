...
#ifndef _LOFTY_COLLECTIONS_VECTOR_0_HXX

#ifndef _LOFTY_NOPUB
   #define _LOFTY_NOPUB
   #define _LOFTY_COLLECTIONS_VECTOR_0_HXX
#endif

#ifndef _LOFTY_COLLECTIONS_VECTOR_0_HXX_NOPUB
#define _LOFTY_COLLECTIONS_VECTOR_0_HXX_NOPUB
...
Instead of forcing users to choose between std::vector and fixed-size arrays (including std::array), Lofty’s
vector class employs different allocation strategies, and is able to use a dynamically-allocated and -resized
element array, while also taking advantage of an optional fixed-size array embedded into it (when the template
argument embedded_capacity is provided and greater than 0).
...
template <
   typename T,
   bool copy_constructible = _std::_LOFTY_PUBNS is_copy_constructible<T>::value,
   bool trivial = _std::_LOFTY_PUBNS is_trivial<T>::value
>
class vector_impl;

// Partial specialization for non-copyable, non-trivial types.
template <typename T>
class vector_impl<T, false, false> : public complex_vextr_impl, public lofty::_LOFTY_PUBNS noncopyable {
public:
   //! Destructor.
   ~vector_impl() {
...
protected:
   ...
   void assign_move_desc_or_move_items(complex_vextr_impl && src) {
      ...
      type.set_destruct<T>();
      type.set_move_construct<T>();
      complex_vextr_impl::assign_move_desc_or_move_items(type, _std::_pub::move(src));
   }
...
protected:
   ...
   void assign_move_desc_or_move_items(trivial_vextr_impl && rtvi) {
      trivial_vextr_impl::assign_move_desc_or_move_items(_std::_pub::move(rtvi));
   }
...
class vector<T, 0> :
   ...
   public lofty::_LOFTY_PUBNS support_explicit_operator_bool<vector<T, 0>> {
private:
   ...
   static bool const copy_constructible = _std::_LOFTY_PUBNS is_copy_constructible<T>::value;
...
public:
   ...
   */
   vector(vector && src) :
      vector_impl(0) {
      vector_impl::assign_move_desc_or_move_items(_std::_pub::move(src));
   }
   ...
   */
   ...
   @param t
      Element to insert.
   */
   void insert(const_iterator offset, typename _std::_LOFTY_PUBNS remove_const<T>::type && t) {
   ...
   */
   T pop_back() {
      T * back_ptr = &back();
      T back_t(_std::_pub::move(*back_ptr));
      vector_impl::remove(back_ptr, back_ptr + 1);
      return _std::_pub::move(back_t);
   }
   ...
   @param t
      Element to add.
   */
   void push_back(typename _std::_LOFTY_PUBNS remove_const<T>::type && t) {
...
*/
template <typename T, std::size_t left_embedded_capacity, std::size_t right_embedded_capacity>
inline typename _std::_LOFTY_PUBNS enable_if<
   _std::_LOFTY_PUBNS is_copy_constructible<T>::value, vector<T, left_embedded_capacity>
>::type operator+(
...
template <typename T, std::size_t left_embedded_capacity, std::size_t right_embedded_capacity>
inline typename _std::_LOFTY_PUBNS enable_if<
   _std::_LOFTY_PUBNS is_copy_constructible<T>::value, vector<T>
>::type operator+(
   vector<T, left_embedded_capacity> const & left, vector<T, right_embedded_capacity> && right
) {
   return vector<T>(left, _std::_pub::move(right));
}
...
public:
   ...
   */
   vector(vector && src) :
      vector_0(embedded_byte_capacity, _std::_pub::move(src)) {
   }
   ...
   */
   vector & operator=(vector && src) {
      vector_0::operator=(_std::_pub::move(src));
      return *this;
   }
...
#endif //ifndef _LOFTY_COLLECTIONS_VECTOR_0_HXX_NOPUB

#ifdef _LOFTY_COLLECTIONS_VECTOR_0_HXX
   #undef _LOFTY_NOPUB

   namespace lofty { namespace collections {

   using _pub::vector;
   using _pub::vector_const_iterator;
   using _pub::vector_iterator;
...
#endif //ifndef _LOFTY_COLLECTIONS_VECTOR_0_HXX

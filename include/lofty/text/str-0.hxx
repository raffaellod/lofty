...
#ifndef _LOFTY_TEXT_STR_0_HXX

#ifndef _LOFTY_NOPUB
   #define _LOFTY_NOPUB
   #define _LOFTY_TEXT_STR_0_HXX
#endif

#ifndef _LOFTY_TEXT_STR_0_HXX_NOPUB
#define _LOFTY_TEXT_STR_0_HXX_NOPUB
...
class LOFTY_SYM sstr<0> :
   ...
   public lofty::_LOFTY_PUBNS support_explicit_operator_bool<str> {
private:
...
public:
   ...
   private:
      ...
      typedef _std::_LOFTY_PUBNS unique_ptr<
         char_t const[],
         memory::_LOFTY_PUBNS conditional_deleter<char_t const[], memory::_LOFTY_PUBNS freeing_deleter>
      > pointer;
   ...
   public:
      ...
      typedef std::ptrdiff_t difference_type;
      typedef _std::_LOFTY_PUBNS random_access_iterator_tag iterator_category;
...
public:
   ...
   */
   sstr(str && src) :
      vextr_impl(0) {
      vextr_impl::assign_move_desc_or_move_items(_std::_pub::move(src));
   }
   ...
   */
   sstr(lofty::_LOFTY_PUBNS external_buffer_t const &, char_t const * s) :
      vextr_impl(0, s, s + text::_pub::size_in_chars(s), true) {
   }
   ...
   */
   sstr(lofty::_LOFTY_PUBNS external_buffer_t const &, char_t const * src, std::size_t src_char_size) :
   ...
   */
   ...
   str & operator=(char_t const (& src)[src_size]) {
      str src_str(src);
      operator=(_std::_pub::move(src_str));
      return *this;
   }
   ...
   */
   collections::_LOFTY_PUBNS vector<std::uint8_t> encode(encoding enc, bool add_nul_term = false) const;
   ...
   */
   ...
   /*! Prints new contents into the string using io::text::_LOFTY_PUBNS ostream::print().
   ...
   */
   void set_from(
      _std::_LOFTY_PUBNS function<std::size_t (char_t * chars, std::size_t chars_max)> const & read_fn
   );
   ...
   */
   void set_size_in_chars(std::size_t new_size, bool clear_ = false) {
      ...
      if (clear_) {
         prepare_for_writing();
         memory::_pub::clear(data(), new_size);
      }
   }
...
public:
   //! Default constructor.
   sstr() :
      _pub::str(embedded_byte_capacity) {
   }
   ...
   @param src
      Source object.
   */
   sstr(_LOFTY_PUBNS str && src) :
      _pub::str(embedded_byte_capacity, _std::_pub::move(src)) {
   }
   ...
   */
   ...
   sstr(sstr<src_embedded_capacity> const & src) :
      _pub::str(embedded_byte_capacity, src.str()) {
   }
   ...
   */
   template <std::size_t src_size>
   sstr(char_t const (& src)[src_size]) :
      _pub::str(embedded_byte_capacity, src) {
   }
   ...
   @return
      *this.
   */
   sstr & operator=(_LOFTY_PUBNS str && src) {
      _pub::str::operator=(_std::_pub::move(src));
      return *this;
   }
   ...
   */
   ...
   sstr & operator=(char_t const (& src)[src_size]) {
      _pub::str::operator=(src);
      return *this;
   }

   using _LOFTY_PUBNS str::operator[];
#ifdef LOFTY_CXX_EXPLICIT_CONVERSION_OPERATORS
   using _LOFTY_PUBNS str::operator bool;
#else
   using _LOFTY_PUBNS str::operator lofty::_pvt::explob_helper::bool_type;
#endif
   ...
   */
   sstr & operator+=(char ch) {
      _pub::str::operator+=(host_char(ch));
      return *this;
   }
#endif
   ...
   */
   ...
   using _LOFTY_PUBNS str::append;
   using _LOFTY_PUBNS str::begin;
   using _LOFTY_PUBNS str::c_str;
   using _LOFTY_PUBNS str::capacity;
   using _LOFTY_PUBNS str::cbegin;
   using _LOFTY_PUBNS str::cend;
   using _LOFTY_PUBNS str::clear;
   using _LOFTY_PUBNS str::crbegin;
   using _LOFTY_PUBNS str::crend;
   using _LOFTY_PUBNS str::data;
   using _LOFTY_PUBNS str::data_end;
   using _LOFTY_PUBNS str::encode;
   using _LOFTY_PUBNS str::end;
   using _LOFTY_PUBNS str::ends_with;
   using _LOFTY_PUBNS str::find;
   using _LOFTY_PUBNS str::find_last;
   using _LOFTY_PUBNS str::format;
   using _LOFTY_PUBNS str::index_from_char_index;
   using _LOFTY_PUBNS str::insert;
   using _LOFTY_PUBNS str::iterator_from_char_index;
   using _LOFTY_PUBNS str::rbegin;
   using _LOFTY_PUBNS str::rend;
   using _LOFTY_PUBNS str::replace;
   using _LOFTY_PUBNS str::set_capacity;
   using _LOFTY_PUBNS str::set_from;
   using _LOFTY_PUBNS str::set_size_in_chars;
   using _LOFTY_PUBNS str::size;
   using _LOFTY_PUBNS str::size_in_bytes;
   using _LOFTY_PUBNS str::size_in_chars;
   using _LOFTY_PUBNS str::starts_with;
   ...
   @return
      *this.
   */
   _LOFTY_PUBNS str const & str() const {
      return *this;
   }
   ...
   @return
      this.
   */
   _LOFTY_PUBNS str * str_ptr() {
      return this;
   }

   using _LOFTY_PUBNS str::substr;
};
...
inline sstr<left_embedded_capacity> operator+(sstr<left_embedded_capacity> && left, char right) {
   return operator+(_std::_pub::move(left), host_char(right));
}
...
inline sstr<right_embedded_capacity> operator+(char left, sstr<right_embedded_capacity> && right) {
   return operator+(host_char(left), _std::_pub::move(right));
}
...
template <>
struct LOFTY_SYM hash<lofty::text::_LOFTY_PUBNS str> {
   std::size_t operator()(lofty::text::_LOFTY_PUBNS str const & s) const;
};

template <std::size_t embedded_capacity>
struct hash<lofty::text::_LOFTY_PUBNS sstr<embedded_capacity>> : public hash<lofty::text::_LOFTY_PUBNS str> {
};
...
#endif //ifndef _LOFTY_TEXT_STR_0_HXX_NOPUB

#ifdef _LOFTY_TEXT_STR_0_HXX
   #undef _LOFTY_NOPUB

   namespace lofty {

   using _pub::external_buffer;
   using _pub::external_buffer_t;
...
#endif //ifndef _LOFTY_TEXT_STR_0_HXX

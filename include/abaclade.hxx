...
namespace abc {
   ...
   Abaclade on non-compliant compilers such as MSC16) is the preferred character type across
   Abaclade; this means that the full range of valid Unicode code points is supported through the
   entire framework.
   ...
   manipulated as such. The internal representation is in reality either UTF-8 or UTF-16, but this
   is for the most part abstracted away from Abaclade-based code.

   While char32_t is the preferred character type, many functions/methods are available with
   overloads accepting C++ single-byte character literals, both for convenience and performance
   reasons. The character type used internally by the abc::text::*str classes is available as
   abc::text::char_t, and the class abc::text::str_traits allows lower-level manipulation of
   abc::text::char_t character arrays.
   ...
   ABC_CHAR('ç')). Additionally, all Unicode code points can be expressed as integers cast to
   char32_t (hexadecimal notation is recommended for consistency with the U+nnnnnn syntax for code
   points; e.g. char32_t(0x23456) for U+23456).

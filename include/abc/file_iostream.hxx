...
//    sector, and the size of the pointed buffer must be a multiple of that same size (as required
//    by at least Win32).
//
...
//    └─────────────────┘
//
//    ┌─────────────────┐   The client read 7 bytes, and the remaining «h» byte is part of a multi-
//    │               h │   byte sequence.
//    └─────────────────┘
//
//    At this point, the only way to unlock this situation would be to temporarily take «h» out of
//    the buffer, refill the buffer, take from it as many bytes as are necessary to complete the
//    character it belongs to, and then resume transcoding the remaining bytes. If it sounds
//    complicated in words, the code will be even worse.
...
//
//    ┌───────────────────────────┐   The read buffer has just been filled via an OS read() API
//    │           a b c d e f g h │   call.
//    └───────────────────────────┘
//
//    ┌───────────────────────────┐   The client read 7 bytes, and the remaining «h» byte is part of
//    │                         h │   a multi-byte sequence.
//    └───────────────────────────┘  
//
//    ┌───────────────────────────┐   To accommodate a buffer refill, the remaining bytes are moved
//    │         h                 │   to the last position before the aligned pointer that will be
//    └───────────────────────────┘   passed to the OS read() API call.
//
...
//    └─────────────────┘
//
//    What happens now if the client wants to re-read these bytes in a different encoding, say
//    UTF-32 (perhaps the first few bytes of the file indicated that the encoding was different than
//    the apparent one), so that the first character actually comprises the bytes «h i j k»?
...
//
//    To avoid this last issue, the unread buffer is simply a portion of the read buffer. So,
//    resuming from the last buffer refill:
//
//    ┌───────────────────────────┐   Upon a new client request, the buffer is filled again via an
//    │           i j k l m n o q │   API call.
//    └───────────────────────────┘
//
//    ┌───────────────────────────┐   The client actually only wanted 2 bytes; the rest is kept in
//    │               k l m n o q │   the buffer.
//    └───────────────────────────┘
//
...
//    └───────────────────────────┘
//
//    This allows to make the unread bytes no special case, since they will be processed just like
//    any other bytes in the read buffer.
//
// To satisfy these constraints, the read buffer is actually only a portion of the larger memory
// block allocated for it, so that there is a memory chunk immediately preceding the read buffer,
// ready for use in the above scenarios.
//
// A good way to test that the code handling the read buffer behaves correctly, is to artificially
// reduce the read buffer size to ludicrous values, and check that exceptions are consciously
// thrown, instead of getting a segfault or memory corruption, in file_istream tests.
